...
module.exports = (api) => {
  ...
  api.metasync.flow = (
    ...
    let type, timeout, composer, funcs;
    ...
    fn.paused = true;
    fn.type = type;
    fn.clone = () => api.metasync.flow(fns);
    ...
    fn.cancel = () => {
      canceled = true;
      if (!fn.paused) finish(new Error('Flow canceled'));
    };
  ...
  api.metasync.sequential = (
    ...
    function next(err, result) {
      if (fn.name) data[fn.name] = err || result;
      if (err) return done(err);
      if (++i === len) return done(null, data);

      fn = fns[i];
      if (Array.isArray(fn)) api.metasync.flow(fn)(data, next);
