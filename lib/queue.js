...
function Queue(concurrency) {
  ...
  this.onFailure = null;
  this.onDrain = null;
  this.lunchTime = 10000;
  this.lunch = false;
  this.workWithoutLunch = 0;
  this.lunchAllowed = true;
}

const QUEUE_TIMEOUT = 'Metasync: Queue timed out';
const QUEUE_DINNER = [
  'Man, we`re having lunch. Don`t you see?',
  'Don`t interfere, we`re having lunch. Wait please.',
  'It is obedniy pereryv now. Are you blind?',
  'We`re on dinner next few hours.',
  'Stupid mortals! Can I eat my dinner in silence?',
  'Oh, how I hate this job. They all think it`s easy but it is not!',
  'Leave me alone, I`m on a lunch break!',
  'Muzchina, I won`t do anything except any thing I want to do since this break! Now leave me alone.',
  'Kogda ya yem, ya gluh i nem',
  'This damned bastards! Will they let me drink my tea and rest?!',
  'No, I won`t do this while I have a break.',
  'If you dislike something you can go away.',
  'Keep calm and don`t interfere me.',
  'I`m afraid, but no. I won`t up my ass and solve your problems while lunchtime.',
  '*gossiping* Did you hear what Liuda`s husband did...',
  'My tea is almost cold and it is becouse I`m listening your stupid complains!',
  'OBIED, WI CHTO NIE WIDITE???',
  '*watching melodramas*',
  '*playing Facebook games*',
  'I will go to doctor soon so be quickiest please.',
  'Ha-ha, those worms waiting while we finish our infinity lunch...',
];
...
Queue.prototype.next = function(task) {
  if (this.lunch) {
    const phrase = QUEUE_DINNER[Math.floor(
      Math.random() * QUEUE_DINNER.length
    )];
    console.log('\x1b[0;34mPERSONAL:\x1b[0m', phrase);
    return this;
  }
  ...
  this.onProcess(item, (err, result) => {
    ...
    this.count--;
    this.workWithoutLunch++;
    if (this.workWithoutLunch >= this.concurrency && this.tasks.length)
      this.lunchBreak();
...
Queue.prototype.setLunchTime = function(lunchtime = 10000) {
  if (lunchtime) {
    this.lunchtime = lunchtime;
    return this;
  }
  console.log('\x1b[0;31mWe`re not slaves!\x1b[0m');
  this.lunchtime = Math.pow(10, 10);
  return this;
}

Queue.prototype.disableLunch = function() {
  this.lunchAllowed = false;
  return this;
}

Queue.prototype.enableLunch = function() {
  this.lunchAllowed = true;
  return this;
}

Queue.prototype.lunchBreak = function() {
  if (!this.lunchAllowed) return this;
  if (!this.lunch) {
    console.log('\x1b[0;31mOBIDNYA PERERVA 1 HODYNA\x1b[0m');
    let lunch = setTimeout(() => {
      clearTimeout(lunch);
      lunch = null;
      this.lunch = false;
      this.workWithoutLunch = 0;
      console.log('\x1b[0;32mPERSONAL IS WORKING AGAIN.\x1b[0m');
      this.takeNext();
    }, this.lunchTime);
    this.lunch = true;
  }
