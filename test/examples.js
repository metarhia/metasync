...
metatests.test('asynchronus filter', test => {
  ...
  const expectedResult = [
    'Lorem', 'ipsum', 'sit', 'amet', 'elit', 'sed',
    'do', 'eiusmod', 'tempor', 'ut', 'labore', 'et',
  ];
...
metatests.test('timeout', test => {
  metasync.timeout(200, done => {
    ...
    const expectedErr = new Error('Metasync: asynchronous function timed out');
    test.isError(err, expectedErr);
    test.end();
  });
});
